...
- 값의 타입이 **반드시 명시** 되어야 함
- 전역 스코프를 포함한 어떤 스코프에서도 선언 가능 (코드의 많은 부분에서 알 필요가 있는 값에 유용)
...
- 루프에 **루프 라벨(loop label)** 을 명시하면 `break` 나 `continue`와 함께 이 키워드 들이 바로 바깥쪽 루프 대신 라벨이 적힌 특정한 루프에 적용
...
#### while을 이용한 조건 반복문
반복문 내에서 조건을 검사하여 조건문이 `true`인 동안에는 계속 반복하는 반복문
