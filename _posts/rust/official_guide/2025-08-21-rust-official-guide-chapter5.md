...
`println!` 매크로에는 여러 출력 형식을 사용할 수 있습니다. 그리고 기본 형식인 `{}`로 지정할 땐 Display라는 최종 사용자를 위한 출력 형식을 사용하는데 여태 사용했던 기본 타입들은 Display가 기본적으로 구현되어 있었습니다. 하지만 구조체의 경우 중간중간 쉼표를 사용하거나, 중괄호를 출력하거나, 필드 일부를 생략하는 등 여러 상황에 우리가 원하는 걸 임의로 예상해서 제공하려 들지 않기 때문에, 구조체에는 `println!` 및 `{}`(자리표시자)와 함께 사용하기 위한 **Display 구현체가 기본 제공되지 않습니다.**
