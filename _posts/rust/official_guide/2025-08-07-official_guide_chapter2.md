...
- 위 에러의 핵심은 **일치하지 않는 타입(mismatched type)** 이 있음에 대해 설명
    ...
    - 만약 let guess = String::new()를 작성한다면 러스트는 guess가 String 타입이어야 함을 추론할 수 있으므로 타입을 작성하지 않아도 됨
    - 위 코드에서 secret_number는 정수형이며 타입 정보를 다른 곳에 추가하지 않았으므로 러스트는 기본적으로 i32라고 생각함
...
- `let guess: u32 = guess.trim().parse().expect("Please type a number!");`
    - `guess.trim().parse().expect()` 
        - 입력값을 문자열로 가지고 있던 원래 guess를 참조하고, String인스턴스의 trim 메서드를 이용해 처음과 끝 부분의 공백 문자들을(`\n` 혹은 `\r\n`) 제거(숫자형 데이터만 저장할 수 있는 u32와 문자열을 비교할 수 있게 하기 위해)
    ...
    - `let guess: u32`
        ...
        - `u32` : 부호가 없는 32비트의 정수
        - 위의 변수 타입 명시를 통해 `guess.cmp(&secret_number)` 에서 `secret_number`의 타입도 u32이어야 한다고 추론할 수 있게 됨 
    - `expect(msg...)`
        - 만약 `parse()` 가 문자열로부터 정수를 만들어낼 수 없어 Err Result 배리언트를 반환한다면, `expect`호출은 게임을 멈추고 제공한 메시지(`msg...`)를 출력

### 프로그램 실행
`    24  ` 와 같이 추릿값 앞 뒤로 빈 값을 넣더라도 프로그램은 추릿값을 `24`로 판단할 수 있게 됨
```bash
$ cargo run
   Compiling guessing_game v0.1.0 (C:\projects\rust_learn\guessing_game)
